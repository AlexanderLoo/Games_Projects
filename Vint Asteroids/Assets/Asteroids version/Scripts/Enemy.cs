using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class Enemy : MonoBehaviour {

	private Rigidbody _rigidbody;
	private GameObject player;
	private AudioSource audioExplosion;
	//private ScoreManager scoreDisplay;
	public GameObject explosion, playerExplosion;
	public GameObject[] smallEnemy, midEnemy;
	public float speed;
	public int score;
	public bool big,mid;

	void Start(){

		audioExplosion = GetComponent<AudioSource> ();
		_rigidbody = GetComponent<Rigidbody> ();
		//player = GameObject.FindGameObjectWithTag ("Player");
		//scoreDisplay = GameObject.Find ("Score").GetComponent<ScoreManager> ();
		_rigidbody.velocity = transform.up * speed;
	}

	void OnTriggerEnter(Collider other){

		// si colisiona con el player le restamos al player una vida
		if (other.CompareTag ("Player")) {

			player.GetComponent<Life> ().life -= 1;
		}

		if (other.CompareTag ("Bullet")) {
			//si el enemigo es grande, instanciamos 2 enemigos medianos(aleatorios) en el mismo sitio pero con rotación aleatorios 
			if (big) {
				for (int i = 0; i < 2; i++) {
					Quaternion newQuaternion = Quaternion.Euler (0, 0, Random.Range (0, 360));
					Instantiate (midEnemy[Random.Range(0,midEnemy.Length)], transform.position, newQuaternion);
				}
			}
			//En caso que un enemigo mediano es destruido, isntanciamos 2 enmigos pequeños
			if (mid) {
				for (int i = 0; i < 2; i++) {
					Quaternion newQuaternion = Quaternion.Euler (0, 0, Random.Range (0, 360));
					Instantiate (smallEnemy[Random.Range(0,smallEnemy.Length)], transform.position, newQuaternion);
				}
			}
			//scoreDisplay.AddScore (score);
			Instantiate (explosion, transform.position, transform.rotation);
			audioExplosion.Play ();
			Destroy (other.gameObject);
			Destroy (gameObject);
		}
	}
}
